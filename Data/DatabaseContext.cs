...
public class DatabaseContext
{
    ...
    // —— Model CRUD ——
    public Task<int> InsertModelAsync(Model m)
    {
        return Connection.InsertAsync(m);
    }

    public Task<List<Model>> GetAllModelsAsync()
    {
        return Connection.Table<Model>().ToListAsync();
    }

    public Task<int> UpdateModelAsync(Model m)
    {
        return Connection.UpdateAsync(m);
    }

    public Task<int> DeleteModelAsync(Model m)
    {
        return Connection.DeleteAsync(m);
    }

    // —— ModelFile CRUD ——
    public Task<int> InsertModelFileAsync(ModelFile f)
    {
        return Connection.InsertAsync(f);
    }

    public Task<List<ModelFile>> GetFilesByModelAsync(string modelId)
    {
        return Connection.Table<ModelFile>().Where(f => f.ModelId == modelId).ToListAsync();
    }

    // —— ModelTag CRUD ——
    public Task<int> InsertModelTagAsync(ModelTag t)
    {
    ...
    {
        return Connection.Table<ModelTag>().Where(t => t.ModelId == modelId).ToListAsync();
    }

    // —— ModelVersion CRUD ——
    public Task<int> InsertModelVersionAsync(ModelVersion v)
    {
        return Connection.InsertAsync(v);
    }

    public Task<List<ModelVersion>> GetVersionsByModelAsync(string modelId)
    {
        return Connection.Table<ModelVersion>().Where(v => v.ModelId.ToString() == modelId).ToListAsync();
    }

    // —— PrintQueueItem CRUD ——
    public Task<int> EnqueuePrintAsync(PrintQueueItem q)
    {
        return Connection.InsertAsync(q);
    }

    public Task<List<PrintQueueItem>> GetAllQueueItemsAsync()
    {
        return Connection.Table<PrintQueueItem>().ToListAsync();
    }

    public Task<int> UpdateQueueItemAsync(PrintQueueItem q)
    {
        return Connection.UpdateAsync(q);
    }

    public Task<int> DeleteQueueItemAsync(PrintQueueItem q)
    {
        return Connection.DeleteAsync(q);
    }
}